...
export const initRoleBacks = () => {
  ...
  roots.forEach((root) => {
    try {
      ...
      const spawnSpark = (payload: { x: number; y: number }) => {
        const host = root.querySelector<HTMLElement>("[data-sparks]");
        if (!host) return;
        const spark = document.createElement("span");
        spark.className = "builder-spark";
        spark.style.left = `${payload.x}px`;
        spark.style.top = `${payload.y}px`;
        host.appendChild(spark);
        setTimer(() => spark.remove(), 520);
      };
          ...
          if (!prefersReducedMotion() && assemblyHost && assemblyHost.clientWidth > 10) {
            requestAnimationFrame(() => {
              builderCleanup = attachBuilderForge(assemblyHost, {
                onSnap: () => {
                  const bounds = assemblyHost.getBoundingClientRect();
                  spawnSpark({ x: Math.random() * bounds.width, y: Math.random() * bounds.height });
                },
              });
            });
          }
      ...
      const stopEffects = () => {
        ...
        const sparkHost = root.querySelector<HTMLElement>("[data-sparks]");
      ...
      const triggerFlipReveal = () => {
        ...
        if (roleId === "coder") {
          ...
          setTimer(() => root.classList.remove("is-blueprint-draw"), 800);
        } else if (roleId === "builder") {
          root.classList.add("is-spark-burst");
          setTimer(() => root.classList.remove("is-spark-burst"), 500);
          // Spawn entrance sparks
          const host = root.querySelector<HTMLElement>("[data-sparks]");
          if (host) {
            const bounds = host.getBoundingClientRect();
            for (let i = 0; i < 6; i++) {
              setTimer(() => spawnSpark({ x: Math.random() * bounds.width, y: Math.random() * bounds.height }), i * 60);
            }
          }
        } else if (roleId === "notebooker") {
        ...
        if (document.hidden) {
          ...
          if (roleId === "driver") {
            ...
            const idleSparks = () => {
              ...
              spawnSpark({ x: Math.random() * bounds.width, y: Math.random() * bounds.height });
              setTimer(idleSparks, randMs(1400, 900));
            };
            idleSparks();
          } else if (roleId === "notebooker") {
            const ghostPage = root.querySelector<HTMLElement>(".notebook-ghostpage");
